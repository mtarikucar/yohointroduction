...
function Home() {
    ...
    return (<>
            {isMobile ? (// Mobile Version
                ...
                <div className="h-screen w-full overflow-hidden bg-[#07051D]">
                    <div
                        className={`h-full ${lockScroll ? "overflow-hidden" : "overflow-y-scroll"} snap-mandatory snap-y no-scrollbar scroll-smooth`}>
                        <div className="snap-center h-full">
                            <Carousel setLockScroll={setLockScroll}/>
                        </div>
                        {navListMenuItems.map(({title, items, introduction}, i) => (
                            <div key={i} id={title} className="snap-center min-h-screen">
